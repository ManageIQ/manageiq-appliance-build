...
describe Build::Cli do
  context "#parse" do
    it "only (default)" do
      expect(described_class.new.parse(%w()).options[:only]).to match_array %w(vsphere ovirt openstack hyperv azure vagrant gce)
    end

    it "all" do
      expect(described_class.new.parse(%w(-o all)).options[:only]).to match_array %w(vsphere ovirt openstack hyperv azure vagrant gce)
    end
